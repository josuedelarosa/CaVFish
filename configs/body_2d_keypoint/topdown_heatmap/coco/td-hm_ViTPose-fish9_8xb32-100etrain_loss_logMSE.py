...
codec = dict(
    ...
    # type='UDPHeatmap', input_size=(512, 384), heatmap_size=(128, 96), sigma=2)
...
model = dict(
    ...
    head=dict(
        type='HeatmapHeadPheno',   # <-- nuestro head extendido
        in_channels=768,           # ViT-Base -> 768
        out_channels=20,           # 20 KPs
        loss=dict(type='KeypointLogMSELoss', 
        use_target_weight=True,
        skip_empty_channel=False,
        eps=1e-6,
        loss_weight=1.0),
        ...
        alpha_pheno=0  # peso de la pérdida fenotípica
    ),
    test_cfg=dict(
        flip_test=False,
        flip_mode='heatmap',
...
#     dict(type='LoadImage'),
#     dict(type='GetBBoxCenterScale'),
#     # Geometric jitter around bbox center (rotate/scale/shift)
#     dict(
#         type='RandomBBoxTransform',
#         rotate_factor=0,                 # degrees
#         scale_factor=(0.85, 1.15),        # inclusive range
#         shift_factor=0.12 # fraction of bbox width/height
#     ),
#     dict(type='TopdownAffine', input_size=codec['input_size'], use_udp=True),

#     # Now augmentations that don’t depend on bbox_scale
#     dict(type='RandomFlip', direction='horizontal', prob=0.5),

#     dict(type='SafeRotateBackoff', max_degree=20, p=0.7),
#     # dict(type='LocalGridMaskKPs', target_indices=[0,1,2,3,4,5], box_size=96, per_kp_prob=0.25, p_img=0.5),
#     dict(type='AlbumentationsColorNoise',
#          noise_var=(5.0, 20.0), p_noise=0.25,
#          brightness=0.25, contrast=0.25, saturation=0.20, hue=0.03, p_color=0.70),
...
############ WORKING DIR
work_dir = '/data/Pupils/Josue/weights/Fish/ViTPose_20kpt_Losspheno_Base_log(MSE)_noPheno'